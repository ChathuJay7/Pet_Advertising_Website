...
<?php
 ...
 $search = '';
  ...
  if (isset($_GET['search']))
  {
    ...
    $query = "SELECT * FROM advertisement WHERE (Category LIKE '%{$search}%' or Title LIKE '%{$search}%') AND IsApproved='false' AND IsDeleted = 0 ORDER BY AddId DESC";
  }

 else
  {
    $query = "SELECT * FROM advertisement WHERE IsApproved='false' AND IsDeleted = 0 ORDER BY AddId DESC";
  }
  ...
  if($users)
  {
    while($user = mysqli_fetch_assoc($users))
    {
      ...
      $advertisement_list .= "<td> <a href = \"approve_advertisement.php?add_id={$user['AddId']}&&user_id={$user_id}\"onclick=\"return confirm('Are you sure?');\"><button style=\"width:90px;color:white;background-color:#3A3B3C;border:none;border-radius:5px\">Approve</button></a> </td>";
      $advertisement_list .= "<td> <a href = \"advertisement_delete_pending.php?add_id={$user['AddId']}&&user_id={$user_id}\"onclick=\"return confirm('Are you sure?');\"><button style=\"width:90px;color:white;background-color:#3A3B3C;border:none;border-radius:5px\">Delete</button></a> </td>";
      $advertisement_list .= "</tr>";
    }
  }
  ...
  <div id="content" class="content">
    <div class="home-content">
      ...
      <div class = "search">
        ...
        <form action="advertisement_pending.php?user_id=<?php echo $user_id ?>" method="get">
        ...
        <a  href="advertisement_pending.php?user_id=<?php echo $user_id ?>"><button>REFRESH</button></a>
