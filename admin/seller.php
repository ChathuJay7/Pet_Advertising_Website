...
<?php
 ...
 //Search the User
  if (isset($_GET['search']))
  {
  	...
  	$query = "SELECT * FROM seller WHERE (FirstName LIKE '%{$search}%' or LastName LIKE '%{$search}%' or Email LIKE '%{$search}%' ) AND IsDeleted = 0 ORDER BY FirstName";
  }

 else
  {
  	$query = "SELECT * FROM seller WHERE IsDeleted = 0 ORDER BY FirstName";
  }
  ...
  if($users)
  {
      ...
      $user_list .= "<td> {$user['Location']} </td>";
  		...
  		$user_list .= "<td> <a href=\" seller_profile_view.php?seller_id={$user['ID']}&&user_id=$user_id\"><button style=\"width:90px;color:white;background-color:#3A3B3C;border:none;border-radius:5px\">View</button></a></td>";
        $user_list .= "<td> <a href = \"delete_seller.php?seller_id={$user['ID']}&&user_id=$user_id\"onclick=\"return confirm('Are you sure?');\"><button style=\"width:90px;color:white;background-color:#3A3B3C;border:none;border-radius:5px\">Delete</button></a> </td>";
  		$user_list .= "</tr>";
  	}
  }
 	...
 	<div id="content" class="content">
 		<div class="home-content">
		 	...
		 	<div class = "search">
				...
				<form action="seller.php?user_id=<?php echo $user_id; ?>" method="get">
          ...
          <input type="hidden" name="user_id" value="<?php echo $user_id; ?>">
				...
				<a  href="seller.php?user_id=<?php echo $user_id; ?>"><button>REFRESH</button></a>
